...
产物目录：<用户指定的绝对路径；如果留空，默认使用目标仓库根目录下的 docs/<kernel>/>
...
所有 RFC、设计文档、失败记录、research state、benchmark、HLO 和 XProf 说明都写入同一个产物目录：优先使用用户填写的目录；未填写时默认是 `<目标仓库根目录>/docs/<kernel>/`。绝不写入本 skills 仓库。
